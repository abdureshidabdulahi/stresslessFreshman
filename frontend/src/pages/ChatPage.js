...
function stripRecommendations(text) {
  return text
    .split('\n')
    .filter((line) => !/^\[RECOMMENDATION\]/i.test(line.trim()))
    .join('\n')
    .trim();
}
...
function RecommendationCards({ recs }) {
  const icons = { book: '📚', video: '▶', website: '◎', article: '□', course: '◇', tool: '⚙' };
  return (
...
function MessageBubble({ msg }) {
  ...
  return (
    <div className={`chat-msg ${isUser ? 'chat-msg--user' : 'chat-msg--ai'}`}>
      ...
      {isUser && <div className="chat-msg__avatar chat-msg__avatar--user">You</div>}
...
const PROMPT_CARDS = [
  { icon: '◎', title: 'Explain', prompt: 'Explain quantum computing in simple terms', text: 'quantum computing in simple terms' },
  { icon: '✎', title: 'Write', prompt: 'Write a professional email for a job application', text: 'a professional email for a job application' },
  { icon: '!', title: 'Give me ideas', prompt: 'Give me ideas for a weekend side project', text: 'for a weekend side project' },
  { icon: '▧', title: 'Analyze', prompt: 'Analyze the pros and cons of remote work', text: 'the pros and cons of remote work' },
];
    ...
    <div className="chat-page">
      ...
      <aside className={`chat-sidebar ${sidebarOpen ? 'chat-sidebar--open' : ''}`}>
        <div className="chat-sidebar__brand">
          <div className="chat-sidebar__brand-mark">AI</div>
          <span className="chat-sidebar__brand-name">StressLess AI</span>
          <button className="chat-sidebar__compose-btn" onClick={startNewChat} title="New chat">+</button>
        </div>

        <div className="chat-sidebar__actions">
          <button className="chat-sidebar__action chat-sidebar__action--primary" onClick={startNewChat}>
            <span>+</span>
            New chat
          </button>
        ...
        <div className="chat-sidebar__footer">
          <div className="chat-sidebar__user">
            <span className="chat-sidebar__user-avatar">{initial}</span>
            <span>{firstName}</span>
            <span className="chat-sidebar__chevron">⌄</span>
          </div>
        </div>
      </aside>
      ...
      <div className="chat-main">
        ...
        <div className="chat-header">
          <button className="chat-header__menu-btn" onClick={() => setSidebarOpen(true)}>☰</button>
          ...
          <div className="chat-header__avatar">{initial}</div>
        </div>

        <div className={`chat-thread ${isEmpty ? 'chat-thread--empty' : ''}`}>
          {loadingSession && (
          ...
          {isEmpty && (
            <div className="chat-empty">
              <h2 className="chat-empty__title">{greeting}</h2>
              <p className="chat-empty__sub">How can I help you today?</p>

              <div className="chat-empty__composer">
                <textarea
                  ref={textareaRef}
                  className="chat-empty__textarea"
                  placeholder="Message StressLess AI..."
                  value={input}
                  ...
                  disabled={loading}
                />
                <div className="chat-empty__composer-tools">
                  <button type="button" onClick={() => fileInputRef.current?.click()} disabled={loading} title="Attach a file">📎</button>
                  
                  <button
                    className="chat-empty__send"
        ...
        {!isEmpty && (
          <div className="chat-input-bar">
            ...
            <textarea
              ...
              placeholder={file ? 'Add a message alongside the file (optional)...' : 'Message StressLess AI...'}
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={handleKeyDown}
              rows={1}
